...
list_of_files = glob.glob("/home/calle/Dropbox/hainich_ongoing_v2/MT/CAZymes/for_fig6/*_kaiju.summary")
list_of_files.sort()
...
for f in list_of_files:
    ...
    ABY1_to_add = 0
    Microgenomates_to_add = 0
    Parcubacteria_to_add = 0
    OC_phyla_to_add = 0
    TN_group_to_add = 0
    FCB_group_to_add = 0
    PVC_group_to_add = 0
        ...
        for line in f_reader:
            ...
            if not line[0].startswith("-"):
                if line[2] != "unclassified" and not line[2].startswith("cannot"):
                    print line[2]
                    if line[2] in ABY1:
                        ABY1_to_add = ABY1_to_add + int(line[1])
                    if line[2] in Microgenomates:
                        Microgenomates_to_add = Microgenomates_to_add + int(line[1])
                    if line[2] in Parcubacteria:
                        Parcubacteria_to_add = Parcubacteria_to_add + int(line[1])
                    if line[2] in OC_phyla:
                        OC_phyla_to_add = OC_phyla_to_add + int(line[1])
                    if line[2] in TN_group:
                        TN_group_to_add = TN_group_to_add + int(line[1])
                    if line[2] in FCB_group:
                        FCB_Group_to_add = FCB_group_to_add + int(line[1])
                    if line[2] in PVC_group:
                        PVC_group_to_add = PVC_group_to_add + int(line[1])
...
#                        taxa_of_interest["Classified"].append(str(100-float(line[0].replace(" ", ""))))
        ...
        taxa_of_interest["ABY1"].append(str(ABY1_to_add))
        taxa_of_interest["Microgenomates"].append(str(Microgenomates_to_add))
        taxa_of_interest["Parcubacteria"].append(str(Parcubacteria_to_add))
        taxa_of_interest["Other candidate phyla"].append(str(OC_phyla_to_add))
        taxa_of_interest["Nitrospinae Group"].append(str(TN_group_to_add))
        taxa_of_interest["FCB Group"].append(str(FCB_group_to_add))
        taxa_of_interest["PVC Group"].append(str(PVC_group_to_add))
    for taxon in taxa_of_interest:
        if len(taxa_of_interest[taxon]) < i:
            taxa_of_interest[taxon].append("0")
    i = i + 1

with open("/home/calle/Dropbox/hainich_ongoing_v2/MT/CAZymes/for_fig6/all_substrates.out", "wb") as f_output:
