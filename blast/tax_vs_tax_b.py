...
blast_taxA = NCBIXML.parse(open(sys.argv[1],"rU"))    #blast output from taxon A specific database
blast_taxB = NCBIXML.parse(open(sys.argv[2],"rU"))    #blast output from taxon B specific database
output_taxA = open(sys.argv[3],"w")    #seqs assigned to taxon A
output_taxB = open(sys.argv[4],"w")    #seqs assigned taxon B
output_specific = open(sys.argv[5],"w")  #seqs with no macthes in either A or B
fasta_handle=open(sys.argv[6],"rU")   #fasta used in BLAST search
...
input_xml = open(sys.argv[1],"rU")   #XML blasted against SILVA SSU
output_xml=open(sys.argv[7],"w") #new XML assigned to SSU rRNA
tax_fasta = open(sys.argv[3],"rU")   #seq of SSU rRNA
