...
parser = argparse.ArgumentParser(description = "Hierarchical KEGG category annotation for metatranscriptome datasets")
parser.add_argument("--megan", "-m", help="'MEGAN KEGG annotation output (contigid K-Number)", action="store")
parser.add_argument("--coverage", "-c", help="path to a bbmpap output file containing coverage information", action="store")
parser.add_argument("--annot", "-a", help="path to the output file (default: megan input file + '.kegg'", action="store")    
...
def read_coverage(cov_inf):
    global cov_stats
    with open(cov_inf, "rb") as infile:
            for line in infile:
                if not line.startswith("#"):
                    cov_stats[line.split("\t")[0][:line.split("\t")[0].find(" ")]] = line.split("\t")[5]
...
# Determine the absolue transcript number per K-Number
def calc_atn():
    global knumber_atn, k_ids
    for knumber, contig_ids in k_ids.items():
        knumber_atn[knumber] = 0
        for contig_id in contig_ids:
            knumber_atn[knumber] = float(knumber_atn[knumber]) + float(cov_stats[contig_id])

# Generate a hierarchical KEGG annotation
def generate_KEGG_annot(k_cov):
    global knumber_annot, kegg_lvl2, kegg_lvl3, kegg_lvl4
    for entry, count in k_cov.items():
...
# the output
def calculate_rta(kegg_annotation, kegg_mapping):
    global kegg_lvl2_order, kegg_lvl3_order, kegg_lvl4
    total_gene_count = sum(kegg_annotation.values())
    for key, value in kegg_annotation.iteritems():
        rta = (value / total_gene_count) * 100
        kegg_annotation[key] = rta
    if kegg_annotation == kegg_lvl2:
        with open(kegg_mapping+".KEGGLvl2.summary", "wb") as kegg_annot_out:
            kegg_annot_out.write("KEGG-Lvl2 Category" + "\t" + "Relative Abundance (mapped reads) [%]" + "\n")
